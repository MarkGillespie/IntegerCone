...
void ConeGene::geneCone(VectorX &conesK, VectorX &phi, VectorX &MA, int coneMax,
  ...
  // double alpha = 5e-3;
  // bool is_fail = true;
  // OptCandidates candidateKs(P * L * P.transpose() * M_2_PI, MA, KN * M_2_PI,
  // coneMax, sigma); int iiter = 0; while (iiter < 1 && is_fail)
  //{

  //	IntegerL0DR<false> IL(2, LN * M_2_PI * factor * sigma, KN * M_2_PI, 1,
  //lp, candidateKs); 	VectorX s = IL.initOri(Aphi); 	candidateKs.factor = factor;
