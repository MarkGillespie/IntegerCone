...
void faceArea(const OpenMesh::Vec3d *points, const MeshTopo &topo,
...
void vertArea(const OpenMesh::Vec3d *points, const MeshTopo &topo,
  ...
  double fA;
  Vec3d e0, e1;
  for (size_t i = 0; i < topo.f2v.size(); ++i) {
    e0 = points[topo.f2v[i][1]] - points[topo.f2v[i][0]];
    e1 = points[topo.f2v[i][2]] - points[topo.f2v[i][1]];
...
void vertGauss(const OpenMesh::Vec3d *points, const MeshTopo &topo,
...
void faceAngle(const Vec3d *points, const MeshTopo &topo,
               std::vector<Vec3d> &f2a) {
  f2a.clear();
  ...
  for (size_t i = 0; i < f2a.size(); ++i) {
    ...
    e1 = points[topo.f2v[i][2]] - points[topo.f2v[i][1]];
    e1.normalize_cond();
    e2 = points[topo.f2v[i][0]] - points[topo.f2v[i][2]];
    e2.normalize_cond();
...
void laplaceTriplet(const OpenMesh::Vec3d *points, const MeshTopo &topo,
  ...
  trips.clear();
  for (size_t i = 0; i < f2a.size(); ++i) {
    trips.emplace_back(topo.f2v[i][0], topo.f2v[i][0],
                       f2cot[i][1] + f2cot[i][2]);
    trips.emplace_back(topo.f2v[i][1], topo.f2v[i][1],
                       f2cot[i][0] + f2cot[i][2]);
    trips.emplace_back(topo.f2v[i][2], topo.f2v[i][2],
                       f2cot[i][0] + f2cot[i][1]);
...
void laplaceTriplet(const Mesh &mesh,
                    std::vector<Eigen::Triplet<double>> &trips) {
  MeshTopo topo(mesh);
